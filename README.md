...
```
L7  PRESENTATION    mobile magenta + analytics teal (Open Design)
...
L3  KNOWLEDGE GRAPH Cognee subgraph — scope: mobile apps, retention metrics, cohorts, funnels
L2  VERSIONED HIST  Memoria (v0.4.0) — namespace "optimobile"
...
```bash
git clone https://github.com/CSOAI-ORG/optimobile-hive
cd optimobile-hive
...
`https://optimobile.ai/.well-known/agent-card.json` — see `agent-card.json`.
...
FLIP. Weak angle vs Firebase/Crashlytics.
